...
?>
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Data PIC Dokter Gigi</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
...
<form name="myForm" id="myForm" onSubmit="return validateForm()" action="data_pic_doktergigi.php" method="post" enctype="multipart/form-data">
    <center><input type="file" id="filepegawaiall" name="filepegawaiall" /></center>
    <input type="submit" name="submit" value="Import" /><br/>
    <label><input type="checkbox" name="drop" value="1" /> <u>Centang, jika ingin mengosongkan isi data terlebih dahulu.</u> </label>
</form>
<?php 
...
//    jika kosongkan data dicentang jalankan kode berikut
    if($_POST['drop']==1){
//             kosongkan tabel
             $truncate ="TRUNCATE TABLE data_pic_doktergigi";
...
//      setelah data dibaca, masukkan ke tabel pegawai sql
      $query = "INSERT into data_pic_doktergigi (nama_faskes,nama_pic,no_pic)values('$name','$tmp_name','$alamat')";
...
-->
                <br>
                <form action="data_pic_doktergigi.php" method="GET">
					<label>Cari : </label>
                ...
                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
					...
					if(isset($_GET['cari'])){
						$cari = $_GET['cari'];
						$data = mysqli_query("SELECT * FROM data_pic_doktergigi WHERE nama_faskes like '%".$cari."%' OR nama_pic like '%".$cari."%' OR no_pic like '%".$cari."%'");
					}
						else{
						$data = mysqli_query("SELECT * FROM data_pic_doktergigi");
						}
					$no=1;
					...
					<tr>							
							...
							<td>
								...
								<button type="button" class="btn btn-info" onclick="javascript: window.location.href='data_pic_doktergigi_edit.php?id=<?php echo $d['no_reg']; ?>';">Edit</button>
								<button type="button" class="btn btn-danger" onclick="javascript: if(confirm('Apakah anda yakin ingin menghapus??')==true)(window.location.href='data_pic_doktergigi_delete.php?id=<?php echo $d['no_reg']; ?>')">Delete</button>
								</div>
							</td>
					</tr>
					<?php } ?>
                            ...
                            <button type="button" class="btn btn-warning" onclick="javascript: window.location.href='data_pic_doktergigi.php';">Back</button>
